...
class Runtime extends EventEmitter {
    ...
    registerPeripheralExtension (extensionId, extension) {
        // 加载时先卸载前一个设备扩展.
        this.unregisterPeripheralExtension(this.peripheralExtensions.id);
        this.peripheralExtensions.id = extensionId;
        this.peripheralExtensions[extensionId] = extension;
    }

    /**
     * 卸载设备扩展.
     * @param {string} extensionId - 设备扩展ID.
     */
    unregisterPeripheralExtension (extensionId) {
        if (this.peripheralExtensions[extensionId]) {
            this._blockInfo = this._blockInfo.filter(x => x.id !== extensionId);
            this.disconnectPeripheral(extensionId);
            this.peripheralExtensions.blocksXML = null;
